...
if __name__ == '__main__':
    ...
    nxyz = []
    poses = []
    for i in tqdm(range(len(data['cameraFrames']))):
    ...
    self_idx = [i*len(dists)+i for i in tqdm(range(len(dists)))]
    new_dists = np.array([dists_flatten[i] for i in tqdm(range(len(dists)*len(dists))) if i not in self_idx])
    near = max(new_dists.min(), 1e-10)
    far = 1.1 * new_dists.max()


    poses = np.array(poses).reshape(-1, 3*5)
    bds = np.array([[near, far] for _ in tqdm(range(len(poses)))])
